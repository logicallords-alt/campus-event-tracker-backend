...
const fs = require('fs');
const { appendToExcel, generateExcelBuffer } = require('../utils/excelHelper');
...
router.post('/', auth, upload.fields([
  ...
  try {
    ...
    const pptFile = req.files?.['event_ppt']?.[0];
    ...
    const event = new Event({
      ...
      event_banner: bannerFile ? getBase64(bannerFile) : null,
      ppt_url: pptFile ? getBase64(pptFile) : null
    });
    await event.save();
    res.json(event);
  } catch (err) {
...
router.post('/:id/post-submission', auth,
  ...
  async (req, res) => {
  try {
    const { result, certificate_missing_reason, team_member_emails, certificate_drive_link, team_member_drive_links } = req.body;
    const student = await User.findById(req.user.id);
    const event = await Event.findOne({
      _id: req.params.id,
      $or: [
    ...
    if (certificate_drive_link) submission.certificate_drive_link = certificate_drive_link;

    // Per-member drive links
    if (team_member_drive_links) {
      try {
        const driveLinks = JSON.parse(team_member_drive_links); // [{email, drive_link}]
        const existing = submission.team_member_drive_links || [];
        driveLinks.forEach(({ email, drive_link }) => {
          if (!email || !drive_link) return;
          const found = existing.find(d => d.email === email);
          if (found) found.drive_link = drive_link;
          else existing.push({ email, drive_link });
        });
        submission.team_member_drive_links = existing;
    ...
    if (status === 'Certificate Verified') {
      ...
      if (submission) {
        ...
        await Achievement.create({
          ...
          college_name: event.college_name || null,
          ...
          certificate_drive_link: submission.certificate_drive_link || null,
        ...
        if (event.event_type === 'team' && event.team_members?.length > 0) {
          ...
          if (acceptedMembers.length > 0) {
            ...
            for (const tm of acceptedMembers) {
              ...
              // Look up this member's individual drive link; fall back to lead drive link
              const memberDriveEntry = (submission.team_member_drive_links || [])
                .find(d => d.email === tm.email);
              const memberDriveLink = memberDriveEntry?.drive_link || submission.certificate_drive_link || null;

              teamAchievements.push({
                student_id: tmUser ? tmUser._id : student._id,
                event_id: event._id,
                event_name: event.event_name,
                college_name: event.college_name || null,
                event_date: event.event_date,
                ...
                result: submission.result,
                certificate_url: memberCertUrl,
                certificate_drive_link: memberDriveLink,
                is_team_member: true,
                team_name: event.team_name || null,
              });
            }
        ...
        // ── Append to department Excel sheet ──────────────────────────────
        const excelRows = [];
        ...
        excelRows.push({
          ...
          result:                  submission.result,
          certificate_drive_link:  submission.certificate_drive_link || '',
        });

        // Team member rows
        if (isTeam && event.team_members?.length > 0) {
          for (const tm of event.team_members.filter(m => m.status === 'Accepted')) {
            const tmUser = await User.findOne({ email: tm.email });
            const memberDriveEntry = (submission.team_member_drive_links || []).find(d => d.email === tm.email);
            excelRows.push({
              student_name:            tm.name,
              reg_no:                  tm.reg_no,
              year:                    tmUser?.year || student.year || '',
              ...
              team_name:               event.team_name || '',
              result:                  submission.result,
              certificate_drive_link:  memberDriveEntry?.drive_link || submission.certificate_drive_link || '',
            });
          }
        }

        await appendToExcel(student.department, excelRows);
        // ──────────────────────────────────────────────────────────────────

        // Notify lead + accepted team members that certificate is verified
        notify(leadId, '🏆 Certificate Verified!', `Your certificate for "${eventName}" has been verified. Achievement added to your dashboard!`, 'verified', event._id);        for (const tm of (event.team_members || []).filter(m => m.status === 'Accepted')) {
          const tmUser = await User.findOne({ email: tm.email });
        ...
        // Delete GeoPhoto records from DB
        await GeoPhoto.deleteMany({ event_id: event._id });
        // Clear event banner from DB to free space
        await Event.updateOne({ _id: event._id }, { $unset: { event_banner: 1, ppt_url: 1 } });
...
// Download department Excel sheet (faculty only) — generated live from DB
router.get('/export/excel', auth, isFaculty, async (req, res) => {
  try {
    const dept = req.user.department;
    const buffer = await generateExcelBuffer(dept);
    if (!buffer) {
      return res.status(404).json({ message: 'No Excel data yet. Verify at least one certificate first.' });
    }
    const safe = dept.replace(/[^a-zA-Z0-9_-]/g, '_');
    res.setHeader('Content-Disposition', `attachment; filename="achievements_${safe}.xlsx"`);
